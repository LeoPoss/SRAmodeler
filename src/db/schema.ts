...
export const complianceRequirements = sqliteTable("compliance_requirement", {
	...
	standardId: integer("standard_id")
		.notNull()
		.references(() => regulationStandards.id, { onDelete: "cascade" }),
...
export const complianceRequirementAttributes = sqliteTable(
	"compliance_requirement_attribute",
	{
		...
		attributeId: integer("attribute_id")
			.notNull()
			.references(() => evaluationAttributes.id, { onDelete: "cascade" }),
	},
	(table) => ({
...
export const auditAssessments = sqliteTable("audit_assessment", {
	...
	processId: integer("process_id").references(() => businessProcesses.id, {
		onDelete: "cascade",
	}),
});
...
export const assessmentValues = sqliteTable(
	"assessment_value",
	{
		assessmentId: integer("assessment_id")
			.notNull()
			.references(() => auditAssessments.id, { onDelete: "cascade" }),
		attributeId: integer("attribute_id")
			.notNull()
			.references(() => evaluationAttributes.id, { onDelete: "cascade" }),
		processElementId: integer("process_element_id")
			.notNull()
			.references(() => processElements.id, { onDelete: "cascade" }),
		recordedValue: text("recorded_value"),
	},
	(table) => ({
		pk: primaryKey({
			columns: [table.assessmentId, table.attributeId, table.processElementId],
		}),
	}),
);
export const regulationStandardsRelations = relations(
	regulationStandards,
	({ many }) => ({
...
export const complianceRequirementsRelations = relations(
	...
	({ one, many }) => ({
		regulationStandard: one(regulationStandards, {
			fields: [complianceRequirements.standardId],
			references: [regulationStandards.id],
		}),
		complianceRequirementAttributes: many(complianceRequirementAttributes),
	}),
);

export const evaluationAttributesRelations = relations(
...
export const complianceRequirementAttributesRelations = relations(
	complianceRequirementAttributes,
	({ one }) => ({
		complianceRequirement: one(complianceRequirements, {
			fields: [complianceRequirementAttributes.requirementId],
			references: [complianceRequirements.id],
		}),
		evaluationAttribute: one(evaluationAttributes, {
			fields: [complianceRequirementAttributes.attributeId],
			references: [evaluationAttributes.id],
		}),
	}),
);

export const auditAssessmentsRelations = relations(
	auditAssessments,
	({ many }) => ({
		assessmentValues: many(assessmentValues),
	}),
...
export const businessProcessesRelations = relations(
	businessProcesses,
	({ many }) => ({
...
export const processElementsRelations = relations(
	processElements,
	({ one, many }) => ({
		businessProcess: one(businessProcesses, {
			fields: [processElements.processId],
...
export const assessmentValuesRelations = relations(
	assessmentValues,
	({ one }) => ({
		auditAssessment: one(auditAssessments, {
			fields: [assessmentValues.assessmentId],
			references: [auditAssessments.id],
		}),
		evaluationAttribute: one(evaluationAttributes, {
			fields: [assessmentValues.attributeId],
			references: [evaluationAttributes.id],
		}),
		processElement: one(processElements, {
			fields: [assessmentValues.processElementId],
			references: [processElements.id],
		}),
	}),
);
