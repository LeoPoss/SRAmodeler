...
interface MappingRequirement {
  ...
  subcategory: string;
  external_id: string;
  bpmn_mapping: { task: boolean; message_event: boolean; pool: boolean; lane: boolean };
  further_specification: string;
  bpmn_annotation: string;
  bpmn_template: string;
}

function bpmnMappingToApplicableFor(mapping: MappingRequirement["bpmn_mapping"]): string {
  const types: string[] = [];
...
function parseInputType(template: string): { input_type: string; options: string | null } {
  if (template === "BooleanToggle") return { input_type: "BooleanToggle", options: null };
  if (template.startsWith("Dropdown")) {
    const match = template.match(/\[(.*)\]/);
    return { input_type: "Dropdown", options: match ? match[1] : null };
  }
  if (template.startsWith("TextInput")) return { input_type: "TextInput", options: null };
...
async function seed() {
  ...
  db.insert(regulationStandards).values({ title: "ISVS", securityLevel: "v1.0" }).run();
  const standard = db.select().from(regulationStandards).where(eq(regulationStandards.title, "ISVS")).get()!;
  ...
  for (const entry of entries) {
    ...
    const req = db.insert(complianceRequirements).values({
    ...
    const attr = db.insert(evaluationAttributes).values({
    ...
    db.insert(complianceRequirementAttributes).values({
  ...
  const pm = db.insert(businessProcesses).values({
  ...
  db.insert(auditAssessments).values({
    auditType: "To-Be",
    processId: pm.id,
  }).run();

  console.log(`Seeded: 1 standard, ${entries.length} requirements, ${attrCount} attributes, ${attrCount} links`);
  console.log(`Pre-seeded Demo Business Process and default "To-Be" audit assessment`);
}
