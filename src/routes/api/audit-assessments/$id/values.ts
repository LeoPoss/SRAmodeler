...
import { assessmentValues, processElements, businessProcesses, auditAssessments } from "#/db/schema";
  ...
  server: {
    handlers: {
      GET: async ({ params, request }) => {
        ...
        if (!assessment) {
          return new Response(JSON.stringify({ error: "Audit assessment not found" }), {
            status: 404,
      ...
      PUT: async ({ params, request }) => {
        ...
        if (!pmId) {
          ...
          if (latest) {
            ...
            const created = db.insert(businessProcesses).values({ processName: "default" }).returning().get();
            pmId = created.id;
          }
        }
        ...
        if (!pe) {
          pe = db.insert(processElements).values({
            processId: pmId,
        ...
        } else if (body.recordedValue !== null && body.recordedValue !== undefined) {
          db.insert(assessmentValues).values({
